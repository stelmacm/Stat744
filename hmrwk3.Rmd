...
The table I used was from the McCallum paper found in the course website. The raw data was found on Dr. Bolkers website for easier linkage into this code. The table itself is very complicated and contains many abbreviated diseases that make little sense without a legend.
The table also includes alot of extra variables such as MetCode which is similar to ID of the given item. Another unimportant column is the Sampler column which contains only POCIS in every row; simply adding to confusion of the table.
...
data3$Drugs <- revalue(data3$Drugs, c("CFN" = "Caffine", "SUC" = "Sucralose", "TMP" = "Trimethoprim", 
...
#very weird that if you run this line of code more than once without cleaing you get an error

```

In the code above I have done my data manipulation. I took the table and only read the important columns. After this I used the gather funcion to put all the data into 3 columns that contain the Site, the abbreaviated drug name and the value of the drug from the given site. This will allow easier data to graph and allow us to have an easier set to graph.
Following this, I renamed all the abbreviations to their respective drug names. Although this worked it seems inaffective and could probably be improved. I played around with it in several ways because I also wanted to get drug catagories.
...
An intruiging premis would be to put the concentrations as lolipop charts similar to the bar chart. I graphed the following one ontop of another again to see how the chart looks.
...
I like the idea of lolipop and I think that is most suitable for this dataset.
...
I like the idea of different colours to represent different sites and I think this would look excellent if the sucralos was to scale with the rest of the values.